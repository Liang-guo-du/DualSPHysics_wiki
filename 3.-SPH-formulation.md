...
where _diam _is line diameter, _E_ is elasticity modulus and C<sub>int</sub> is the internal damping coefficient. For chains, which have very small internal structural damping, a small amount of structural damping is still included in the model to critically damp non-physical resonances that can occur at the individual segment natural frequencies due to the model discretization. Bending and torsional stiffnesses are neglected. Hydrodynamic damping and added mass are represented using the Morison equation applied to each node. The tangent direction at a node _i_ is approximated as a unit vector aligned between the nodes on either side:
