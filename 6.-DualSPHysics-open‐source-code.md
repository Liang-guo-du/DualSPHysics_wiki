This section provides a brief description of the source files of DualSPHysics v4.2. The source code is freely redistributable under the terms of LGPL v2.1- GNU Lesser General Public License (LGPL) as published by the Free Software Foundation ([www.gnu.org/licenses](http://www.gnu.org/licenses)). 

Thus, users can download the files from DualSPHysics_v4.2/src. 

First, note that a more complete documentation is provided using the documentation system Doxygen ([www.doxygen.org](http:www.doxygen.org)) that can be found in ([www.dual.sphysics.org/doxygen/](http:www.dual.sphysics.org/doxygen/)).  
