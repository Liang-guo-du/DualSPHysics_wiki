...
The solution can be validated against the analytical solution provided in [Tafuni et al. 2018](https://doi.org/10.1016/j.cma.2018.08.004). Using the **MeasureTool** code, the velocity profile along _Z_ at the center of the domain is computed using list of points described in the file _points_oc.txt_. The output results can be compared with the theoretical solution calculated in the Excel file <strong>Analytical_solution_slope_channel.xslx</strong> provided in the test case folder. The comparison can be seen below in Figure 12-X11 and is quite good despite the coarse resolution adopted in this test case.
