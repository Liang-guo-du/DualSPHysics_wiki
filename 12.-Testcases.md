...
<p align="center">
<strong>Figure 12-6-1.</strong> List of inlet/outlet test cases provided with the version 4.4 of DualSPHysics.
</p>

A series of examples is included in the folder <strong>/examples/inletoutlet</strong> to show the capability of inlet and outlet conditions to model fluid flow accurately. The test cases span a large variety of flow conditions, including confined and free-surface flow, zero-gravity flow, wave generation and many others. A short summary of each test case is listed below, after which a more thorough explanation is provided.

- <strong>1_FlowCylinder</strong>: 2-D flow past a circular cylinder under in steady and unsteady mode: Reynolds 20 (steady flow) and Reynolds 100 (unsteady flow)
- <strong>2_OpenChannel</strong>: 2-D open channel flow over an inclined slope at Reynolds = 100
- <strong>3_Reverse Flow</strong>: 2-D flow reversion, where the reversion is shown by means of a floating body immersed in the flow
- <strong>4_Waves_2D</strong>: 2-D wave generation over a sloped beach, where the waves are generated at the inlet using predefined velocities from linear wave theory
- <strong>5_ShapesInlet3D</strong>: 3-D simulation showing capability to generate buffers with different shapes, such as cylindrical or diamond shapes.
- <strong>6_Box4Inlet3D</strong>: 3-D simulation showing capability to utilize several inlets and outlets in a simulation.
- <strong>7_CurrentHull</strong>: 3-D simulation showing the free-surface flow past a ship hull in a narrow channel.