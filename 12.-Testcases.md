...
- <a href="#obc1"><strong>1_FlowCylinder</strong></a>: 2-D flow past a circular cylinder under in steady and unsteady mode: Reynolds 20 (steady flow) and Reynolds 100 (unsteady flow)
- <a href="#obc2"><strong>2_OpenChannel</strong></a>: 2-D open channel flow over an inclined slope at Reynolds = 100
...
The first test case shows the effectiveness of the proposed algorithm in simulating 2-D flow past a circular cylinder at different Reynolds numbers. A cylinder of diameter D = 0.1 m is discretized by dynamic boundary particles and surrounded by water. The fluid is initialized with a constant x-velocity, U = 1 m/s and a null y-velocity. The Reynolds number is defined as Re = U D/<i>nu</i>, where <i>nu</i> the kinematic viscosity in m<sup>2</sup>/s. 
...
The first test case shows the effectiveness of the proposed algorithm in simulating 2-D flow past a circular cylinder at different Reynolds numbers. A cylinder of diameter D = 0.1 m is discretized by dynamic boundary particles and surrounded by water. The fluid is initialized with a constant x-velocity, U = 1 m/s and a null y-velocity. The Reynolds number is defined as Re = U D/<i>nu</i>, where <i>nu</i> the kinematic viscosity in m<sup>2</sup>/s. The left buffer represents the inlet zone, where the y-velocity of the particles set to zero and the x-velocity set to the constant value U. The right buffer has the same dimensions as Buffer 1, however the velocity and density of the outlet particles is obtained by using the ghost nodes. The same extrapolation process is utilized to compute the density, and hence pressure, of all buffer particles. The viscosity is varied to simulate different Reynolds numbers, particularly a steady case (Re = 20) and an unsteady case (Re = 100).
...
<p align="center">
<strong>Figure 12-X9.</strong> Contours of velocity magnitude (top) and vorticity in Y (bottom) for Re = 100.
</p>
...
<p align="center">
<strong>Figure 12-X10.</strong> Contours of velocity magnitude (top) and pressure (bottom) for Re = 100.
</p>
...
<p align="center">
<strong>Figure 12-X11.</strong> Screenshot of velocity magnitude for the case of reverse flow with floating body.
</p>
...
<p align="center">
<strong>Figure 12-X12.</strong> Wave generation using imposed velocity from linear wave theory at the inlet.
</p>
...
<p align="center">
<strong>Figure 12-X13.</strong> Different types of buffer shapes created via the new inlet/outlet algorithm.
</p>
...
<p align="center">
<strong>Figure 12-X14.</strong> Multi-inlet multi-outlet demonstration via 3-D box filled with water using four inlet areas.
</p>
...
<p align="center">
<strong>Figure 12-X15.</strong> Simulation of flow past a boat hull using inlet/outlet buffer in three dimensions.
</p>