...
A series of examples is included in the folder <strong>/examples/inletoutlet</strong> to show how to implement simulations using open boundary conditions. Through these solved examples it is also possible to test the capability of inlet and outlet conditions of modeling fluid flow accurately. Several flow are simulated, including confined and free-surface flow, zero-gravity flow, wave generation and many others. A short summary of each test case is listed below, after which a more thorough explanation is provided.
